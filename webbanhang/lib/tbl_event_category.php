...
class eventCategory
{
	function getArrayCategory($split="==",$id){
		...
		$result =$kw->getRecord('tbl_event_category',$where);	
		while($row=mysql_fetch_array($result)){		
			$ret[] = array($row['categories_id'],$split.$row['categories_name']);
			$getsub =$kw->getRecord('tbl_event_category',"status=1 and parent=".$row['categories_id']);	
	...
	function GetCategory($where)
	{
		...
		for($i=0;$i<$rows;$i++)
		{
			...
			$parent =$kw->getRecord('tbl_event_category',"categories_id=".$row['parent']) ;
			...
			//$arr[]=array($name0,$id1,$parent2,$detail_short3 ,$date_added4,$last_modified5,$status6,$sort7,$image8,$code9,$user,$rowparent['categories_name']);		
		}
		return $arr;
	}
	...
	{
		$kw=new KW_Hook();
		return $kw->CountRecord('tbl_event_category',$where);
	}
	function DEL($id)
	{
		$kw=new KW_Hook();
		$result=$kw->AccessNoneReturn('update tbl_event_category set status=0 where categories_id="'.$id.'"');
		return $result;
	}
	...
	{		
		...
		if($r=="")
		{		
			$str="insert into  tbl_event_category(categories_name,image,parent,sort,status,date_added,last_modified,	doc_categories_desc,useradd,code) values(N'".$CategoryName."',N'".$Category_image."',N'".$cate."',N'".$sort."','1','".mktime()."','".mktime()."',N'".$Categoryinfo."','".$username."','".$code."')"; 			
	...
	{	
		$str="update tbl_event_category set categories_name=N'".$partnerName."',parent=N'".$partnerCate."',sort=N'".$sort."',status=1,last_modified='".mktime()."',doc_categories_desc=N'".$partnerinfo."',useradd='".$username."',code=N'{$code}' where categories_id='".$partner_id."'"; 
		$kw=new KW_Hook();
		...
		if($r=="")
		{
			...
			{
				$selectpartner=$kw->getRecord('tbl_event_category',"categories_id='".$partner_id."'");
	...
	function ADDProductCategory($id,$product)
	{	
		$str="update tbl_event_category set doc_categories_desc=N'".$product."' where categories_id='".$id."'"; 
		$kw=new KW_Hook();
	...
	{		
		$str="update  tbl_event_category set image='".$partner_image."' where categories_id='".$partner_id."'";
		$kw=new KW_Hook();
		$result=$kw->AccessNoneReturn($str);
		return $result;
	}
