...
class ProductCategory {
	...
	function getArrayCategory($split = "==") {
		...
		$result = $kw->getRecord ( 'tbl_product_category', $where );
		while ( $row = mysql_fetch_array ( $result ) ) {
			...
			for($j = 0; $j < $rows; $j ++) {
				...
				$getsub2 = $kw->getRecord ( 'tbl_product_category', "  status=1 and parent=" . $sub ['categories_id'] );
	...
	function GetCategory($where) {
		...
		for($i = 0; $i < $rows; $i ++) {
			...
			$parentresult = $kw->getRecord ( 'tbl_product_category', "categories_id=" . $row ['parent'] );
	...
	//tao duy nhat 1 san pham cho 1 loai
	

	function CountCategory($where) {
		$kw = new KW_Hook ( );
		$num = $kw->CountRecord ( 'tbl_product_category', $where );
		return $num;
	}
	function InsertCategory($categoryName, $categoryinfo, $cate, $category_image, $sort, $username, $tieubieu) {
		...
		if ($r == "") {
			$str = "insert into  tbl_product_category(categories_name,parent,sort,status,date_added,last_modified,categories_description,useradd,indexpage) values(N'" . $categoryName . "',N'" . $cate . "',N'" . $sort . "','1','" . mktime () . "','" . mktime () . "',N'" . $categoryinfo . "','" . $username . "','" . $tieubieu . "')";
	...
	function UpdateCategory($partnerName, $partnerinfo, $partnerCate, $partner_image, $sort, $partner_id, $username, $indexpage) {
		$str = "update tbl_product_category set indexpage='" . $indexpage . "', categories_name=N'" . $partnerName . "',parent=N'" . $partnerCate . "',sort=N'" . $sort . "',status=1,date_added='" . mktime () . "',last_modified='" . mktime () . "',categories_description=N'" . $partnerinfo . "',useradd='" . $username . "' where categories_id='" . $partner_id . "'";
		$kw = new KW_Hook ( );
		$r = $kw->checkUpload ( $partner_image, ".jpg;.png;.jpeg;.gif", 500 * 1024, 0 );
		if ($r == "") {
			$result = $kw->AccessNoneReturn ( $str );
			if ($result && $partner_image ['size'] > 0) {
				$selectpartner = $kw->getRecord ( 'tbl_product_category', "partner_id='" . $partner_id . "'" );
	...
	function UpdateCategoryImage($categories_image, $categories_id) {
		$str = "update  tbl_product_category set categories_image='" . $categories_image . "' where categories_id='" . $categories_id . "'";
		$kw = new KW_Hook ( );
		$result = $kw->AccessNoneReturn ( $str );
		return $result;
	}
	function DeleteCategory($partner_id) {
		$str = "update tbl_product_category set status=0 where categories_id='" . $partner_id . "'";
		$kw = new KW_Hook ( );
		$result = $kw->AccessNoneReturn ( $str );
		return $result;
	}
	function RestoreCategory($partner_id) {
		$str = "update tbl_product_category set status=1 where categories_id='" . $partner_id . "'";
		$kw = new KW_Hook ( );
		$result = $kw->AccessNoneReturn ( $str );
		return $result;
	}
	function DeleteCategoryAdmin($partner_id) {
		$str = "delete from tbl_product_category where categories_id='" . $partner_id . "'";
		$kw = new KW_Hook ( );
		$result = $kw->AccessNoneReturn ( $str );
		return $result;
	}
