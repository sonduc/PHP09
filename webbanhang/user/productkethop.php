...
$product=$productclass->GetProduct("products_id='".$_GET['id']."'") ;
$productkethop=$product[0][22];
...
if($_POST['txtsubmit']==1)
{
	...
	if(isset($_POST['checkbox']))
	{
		...
		$str="";
			...
			{
				if($productkethop!="")
					$str=$productkethop.",".$str;
				$productclass->EditProductKetHop($_GET['id'],$str);
			}
			$kw->Redirect(pathtoweb."admin/CMS.php?page=product");
	}
}
if($productkethop=="")
	$productlist=$productclass->GetProduct('products_status=1  and categories_id=\''.$idcategory.'\'');
else 
	$productlist=$productclass->GetProduct('products_status=1  and categories_id=\''.$idcategory.'\' and products_id not in ('.$productkethop.')');
...
<script language="javascript">
			$(document).ready(function(){ 
				$("#cbcategory").change(function(){
					window.location.href="?page=addproductkh&id="+$("#idevent").val()+"&idcategory="+$("#cbcategory").val();
				});						
			});
</script>
<div class="grid_16">
